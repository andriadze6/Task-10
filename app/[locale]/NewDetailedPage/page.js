                ...
                <div className="NewDetailedPage-Container">
                        <div className="Slider-Container">
                            <div className="Small-ImgDiv">
                                {
                                    imgArray.map((item, index) => {
                                        return(
                                            <div className={`${sliderState.clickAmount === index ? "borderActive" : ""}`} key={index}>
                                                <Image width={70} height={70} className="Small-Img" alt="" src={item}/>
                                            </div>
                                        )
                                    })
                                }
                            ...
                            <div className="Big-ImgDiv">
                                <div style={{transform:`translate3d(-${sliderState.transferX}%, 0, 0)`}} className="Big-Img-Container">
                                    {
                                        imgArray.map((item, index) => {
                                            return(
                                                <div style={{flex: `0 0 calc(100%)`}} key={index}>
                                                    <Image width={500} height={700} className="Big-Img" alt="" src={item}/>
                                                </div>
                                            )
                                        })
                                    }
                                ...
                                <div className="changeSliderButton">
                                    <button onClick={()=>{changeSlider(0)}} className="slider-Button">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6"><path stroke-linecap="round" stroke-linejoin="round" d="M6.75 15.75 3 12m0 0 3.75-3.75M3 12h18"></path></svg>
                                        </button>
                                    <button onClick={()=>{changeSlider(1)}} className="slider-Button">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6"><path stroke-linecap="round" stroke-linejoin="round" d="M17.25 8.25 21 12m0 0-3.75 3.75M21 12H3"></path></svg>
                                    </button>
                                </div>
                            </div>
                        </div>
