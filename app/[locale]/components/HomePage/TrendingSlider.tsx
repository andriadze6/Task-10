...
export default function Slider({ manTrending,womanTrending,kidTrending, newArrival}:homepageType){
    debugger
            ...
            <div className="TSlider-Wrapper">
                <div className="TSlider-Container"
                    ...
                    {category.data.map((item)=>{
                        debugger
                        return(
                            <div key={item.product_ID} style={{flex:`0 0 calc(100% / ${3})`}} className="Item">
                                <div style={{position:"relative"}}>
                                    {
                                       item?.img && Object.entries(item.img).map((key,index,value,)=>{
                                            if(index == 0 && item.img){
                                                debugger
                                                let imgArray = key[1];
                                                return(
                                                    <>
                                                        <Image key={index} className="T-Img" alt="" height={500} width={500} src={imgArray[0]}/>
                                                        <Image key={index} className="T-Img2" alt="" height={500} width={500} src={imgArray[2]}/>
                                                    </>
                                                )
                                            }
                                        })
